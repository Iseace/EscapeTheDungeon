...
public class CircularDoor : MonoBehaviour
{
    ...
    [SerializeField] private bool showBorder = true; // Toggle border visibility
    ...
    [SerializeField] private bool showAttachmentPoints = true; // Show gizmos for attachment points
    ...
    {
        // Calculate attachment points at the sides of the door (in local space)
        float halfWidth = doorWidth / 2f;
        float midHeight = doorHeight / 2f;

        // Left attachment point (middle of left edge)
        LeftAttachmentPoint = new Vector3(-halfWidth, midHeight, 0);
        LeftAttachmentNormal = Vector3.left; // Normal pointing left

        // Right attachment point (middle of right edge)
        RightAttachmentPoint = new Vector3(halfWidth, midHeight, 0);
    ...
    public bool IsPointNearAttachment(Vector3 worldPoint, out Vector3 snapPoint, out bool isLeftSide)
    {
        ...
        if (distToLeft < attachmentRange && distToLeft <= distToRight)
        {
            snapPoint = leftWorld;
            isLeftSide = true;
            return true;
    ...
    private void DetectAndAttachToFloor()
    {
        ...
        // Get door's floor connection point (bottom center)
        Vector3 doorFloorPoint = GetFloorConnectionPointWorld();

        // Store previous attachment
        Circularfloor prevFloor = attachedFloor;
        ...
        // Check each floor for proximity
        foreach (Circularfloor floor in floors)
        {
            if (floor.Type == Circularfloor.RoomType.Floor)
            {
                // Get floor border points and find closest point on border
                ...
                if (dist < closestDist)
                {
                    closestDist = dist;
                    attachedFloor = floor;
                    floorSnapPoint = closestPoint;
                    floorSnapNormal = borderNormal;
                    snapToFloorBorder = true;
                }
            }
        }

        // If attached to a floor, snap to it
        if (snapToFloorBorder && attachedFloor != null)
        {
            SnapToFloorBorder();
            
            // Initialize tracking variables on new attachment
            if (prevFloor != attachedFloor)
            {
                prevFloorPosition = attachedFloor.transform.position;
                prevFloorRotation = attachedFloor.transform.rotation;
                prevFloorWidth = GetFloorWidth(attachedFloor);
    ...
    private float GetClosestPointOnFloorBorder(Circularfloor floor, Vector3 worldPoint, out Vector3 closestPoint, out Vector3 borderNormal)
    {
        ...
        // Floor is in XZ plane at y=0, so we only care about x and z
        ...
        // Find closest point on the border (rounded rectangle)
        Vector3 closestLocal;
        Vector3 normalLocal;

        // Clamp to border with corner rounding
        ...
        if (cornerRadius > 0.01f)
        {
            ...
            if (Mathf.Abs(clampedX) > halfW - cornerRadius && Mathf.Abs(clampedZ) > halfH - cornerRadius)
            {
                inCornerRegion = true;
                // Determine which corner
                cornerCenter = new Vector3(
                    clampedX > 0 ? halfW - cornerRadius : -halfW + cornerRadius,
                    0,
                    clampedZ > 0 ? halfH - cornerRadius : -halfH + cornerRadius
                );
            }
        }
    ...
    private void OnDrawGizmos()
    {
        ...
        // Draw wall attachment points
        if (showAttachmentPoints)
        {
            // Draw left attachment point
